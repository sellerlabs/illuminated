...
trait CrawlerTrait
{
    ...
    /**
     * Add a root if the URL is relative (helper method of the hasLink
     * function).
     *
     * @param  string $url
     *
    ...
    {
        $checkbox = $this->filterByNameOrId($selector,
            "input[type='checkbox']");
    ...
    public function seeIsSelected($selector, $expected)
    {
        $this->assertEquals(
            $expected,
            $this->getSelectedValue($selector),
    ...
    public function callSecure(
        ...
        return $this->response = $this->call($method,
            $uri,
            $parameters,
            $cookies,
            $files,
    ...
    protected function see($text, $negate = false)
    {
        ...
        $this->$method("/({$rawPattern}|{$escapedPattern})/i",
    ...
    protected function seeJsonContains(array $data, $negate = false)
    {
        ...
        foreach (array_sort_recursive($data) as $key => $value) {
            ...
            $this->{$method}(
                ...
                . " JSON fragment [{$expected}] within [{$actual}]."
            );
        }
    ...
    /**
     * Asserts that the response contains the given cookie and equals the
     * optional value.
     *
